local tS = TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local tM = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local ts = game:GetService('ts')
local cg = game:GetService('cg')
local Teams = game:GetService('Teams')
local Players = game:GetService('Players')
local rs = game:GetService('rs')
local tw = game:GetService('tw')
local Lighting = game:GetService('Lighting')
local rt = rs.rt
local lp = Players.lp
local Mouse = lp:GetMouse()
local pg = pg or (syn and syn.protect_gui) or (function() end)
...
tw:Create(Library.BlurEffect, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = Library.BlurSize}):Play()
end
else
local tween = tw:Create(Library.BlurEffect, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = 0})
tween:Play()
...
updateDescendants(sg)
local mobileUI = cg:FindFirstChild("LinoriaMobileUI")
if mobileUI then
...
local RainbowStep = 0
local Hue = 0
table.insert(Library.Signals, rt:Connect(function(Delta)
...
local tween = tw:Create(instance, tweenInfo or tM, {[property] = targetValue})
tween:Play()
return tween
end
...
local Bounds = ts:GetTextSize(Text, Size, Font, Resolution or Vector2.new(1920, 1080))
...
ChangedConn = is.InputChanged:Connect(function(Change)
...
EndedConn = is.InputEnded:Connect(function(EndInput)
...
Library:GiveSignal(sg.DescendantRemoving:Connect(function(Instance)
...
tw:Create(Button, tS, { TextColor3 = Library.AccentColor }):Play()
end)
Button.MouseLeave:Connect(function()
tw:Create(Button, tS, { TextColor3 = Library.FontColor }):Play()
end)
...
local ChangedConn = is.InputChanged:Connect(function(Change)
if Change.UserInputType == Enum.UserInputType.MouseMovement or Change == Input then
...
tw:Create(Label, tS, { TextColor3 = Library.AccentColor }):Play()
end)
Label.MouseLeave:Connect(function()
if Label.TextColor3 ~= Library.FontColor then
tw:Create(Label, tS, { TextColor3 = Library.FontColor }):Play()
end
end)
...
tw:Create(ContainerLabel, tS, { TextColor3 = targetColor }):Play()
end
...
return Key == 'MB1' and is:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
or Key == 'MB2' and is:IsMouseButtonPressed(Enum.UserInputType.MouseButton2)
or Key == 'Touch' and true
else
return is:IsKeyDown(Enum.KeyCode[KeyPicker.Value])
end
else
...
task.wait(0.2)
local Event
Event = is.InputBegan:Connect(function(Input)
local Key
...
tw:Create(Outer, tS, { BackgroundColor3 = Library.AccentColor }):Play()
tw:Create(Label, tS, { TextColor3 = Color3.new(1, 1, 1) }):Play()
end)
Outer.MouseLeave:Connect(function()
tw:Create(Outer, tS, { BackgroundColor3 = Library.SurfaceColor }):Play()
tw:Create(Label, tS, { TextColor3 = Library.FontColor }):Play()
end)
return Outer, Label
end
...
tw:Create(Btn.Label, tS, { TextColor3 = Library.AccentColor }):Play()
Btn.Label.Text = 'Are you sure?'
Btn.Locked = true
local clicked = WaitForEvent(Btn.Outer.InputBegan, 0.5, ValidateClick)
tw:Create(Btn.Label, tS, { TextColor3 = Library.FontColor }):Play()
...
tw:Create(Fill, tM, { Size = UDim2.new(0, X, 1, 0) }):Play()
end
...
ZIndex = 21
Parent = ListOuter
TopImage = 'rbxassetid://8931631581'
BottomImage = 'rbxassetid://8931631581'
...
Button.MouseEnter:Connect(function()
tw:Create(ButtonLabel, tS, { TextColor3 = Library.AccentColor }):Play()
end)
Button.MouseLeave:Connect(function()
if not Table:IsSelected() then
tw:Create(ButtonLabel, tS, { TextColor3 = Library.FontColor }):Play()
end
end)
local Selected
if Info.Multi then
...
tw:Create(ButtonLabel, tS, { TextColor3 = targetColor }):Play()
end
end
...
tw:Create(DropdownArrow, tS, { Rotation = 180 }):Play()
end
function Dropdown:CloseDropdown()
ListOuter.Visible = false
Library.OpenedFrames[ListOuter] = nil
tw:Create(DropdownArrow, tS, { Rotation = 0 }):Play()
end
...
task.spawn(function()
tw:Create(NotifyOuter, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
Size = UDim2.new(0, finalWidth, 0, YSize)
}):Play()
task.wait(Time or 5)
tw:Create(NotifyOuter, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
Size = UDim2.new(0, 0, 0, YSize)
}):Play()
task.wait(0.3)
NotifyOuter:Destroy()
...
tw:Create(WindowLabel, tS, { TextTransparency = 1 }):Play()
task.delay(0.15, function()
WindowLabel.Text = Title
tw:Create(WindowLabel, tS, { TextTransparency = 0 }):Play()
end)
end
...
tw:Create(TabButton, tM, { BackgroundColor3 = Library.SurfaceColor }):Play()
...
tw:Create(TabButton, tM, { BackgroundColor3 = Library.BackgroundColor }):Play()
...
tw:Create(Button, tS, { BackgroundColor3 = Library.BackgroundColor }):Play()
...
tw:Create(Button, tS, { BackgroundColor3 = Library.SurfaceColor }):Play()
...
tw:Create(Outer, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
if Library.Toggled then
task.spawn(function()
local State = is.MouseIconEnabled
...
while Library.Toggled and sg.Parent do
is.MouseIconEnabled = false
local mPos = is:GetMouseLocation()
...
Library:GiveSignal(is.InputBegan:Connect(function(Input, Processed)
...
if is.TouchEnabled then
local MobileGui = Instance.new("sg")
